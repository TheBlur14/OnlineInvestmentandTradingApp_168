			...
			case 8:
				...
				if (pa.size() != 0 && app.size()!=0) {
					for (int i = 0; i < pa.size(); i++) {
						...
						if((app.get(j).getAppID()==pa.get(i).getId()) && (app.get(j).getAppID()==hcp.get(k).getId())) {
						     System.out.println("***************************************");
...
//								    AppointmentDao.deleteById(DB.connect(), apt.get(k).getAppID());
			...
			case 10:
				System.out.println("Thank You using our platform! Your WellBeing is our Trust!");
				System.exit(0);
				break;
			default:
